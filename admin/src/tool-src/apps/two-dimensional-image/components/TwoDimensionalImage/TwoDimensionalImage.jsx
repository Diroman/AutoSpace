...
class TwoDimensionalImage extends Component {
	...
	handleSubmit = (type) => {
		const {
			imageScaleFactor, imageWidth, imageHeight, annotations, entities, rootOptionId, cameraID,
		} = this.state;
		...
		case 'Skip':
			onSkipClick({
				cameraID, url, imageScaleFactor, imageWidth, imageHeight, annotations: denormalizedAnnotations, options: denormalizedOptions,
			});
			break;
		case 'Previous':
			onPreviousClick({
		...
		/*
		axios.post(`http://192.168.31.44:8080/set-polygon`, {info})
			.then(res => {
				console.log(res);
	...
	render() {
		...
		const addButtonUI = (
			...
			>
				{isAdding ? 'Добавление области парковки' : 'Добавить область парковки'}
			</Button>
		);
